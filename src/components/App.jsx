...
export default function App() {
  ...
  const actualNames = () => {
    ...
    const filteredContacts = arrContacts.filter(contact =>
      contact.name.toLowerCase().includes(contactsFiltered)
    );
  ...
  return (
    ...
    >
      <h2>Phone book</h2>
      <ContactForm newContact={pushedContacts} />

      <h2>Contacts</h2>

      <Filter value={filter} parameterTracker={filterContacts} />
      <ContactList
        deleteContact={handlerContactDelete}
        contacts={actualNames()}
      />
    </div>
  );
}
